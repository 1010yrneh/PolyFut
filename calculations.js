...
function applyShadowValues(coeffs) {
    ...
    let baseError = Math.abs(scaled['Defensive Error'] != null ? scaled['Defensive Error'] : 0.03459);
    let baseShot = Math.abs(scaled['Shot Taken'] != null ? scaled['Shot Taken'] : 0.1108);
...
function calculatePerformance(matchStats, currentScore, duration, excludedRanges, position) {
    ...
    matchStats.forEach(stat => {
        ...
        chartData.push({ x: stat.seconds, y: Math.round(runningTotal * 1000) / 1000 });
    });

    if (duration > 0) {
        chartData.push({ x: duration, y: Math.round(runningTotal * 1000) / 1000 });
    }
